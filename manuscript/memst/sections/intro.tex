...
We introduce \textbf{MemSt} (pronounced ``mem-st''), a semantic memory architecture inspired by Git's content-addressable storage model. MemSt treats memory as a versioned, structured repository rather than an ephemeral cache.
...
\begin{itemize}
    ...
    \item \textbf{Four-Tier Memory Hierarchy}: Working (hot), Short-term (recent), Long-term (important), and Archival (historical) tiers with automatic transitions based on access patterns, importance scores, and temporal decay.
    
    \item \textbf{Hybrid Search Architecture}: Native BM25 full-text search, HNSW vector similarity search, and Reciprocal Rank Fusion (RRF) combining both signals with configurable weights.
    
    \item \textbf{Knowledge Graph Extraction v2}: Multi-backend entity and relationship extraction supporting 80+ domain ontologies, with storage options for SQLite (development) and DuckDB (production analytics).
    
    \item \textbf{Sleep-Time Consolidation}: Asynchronous background pipeline for memory compaction, entity merging, and knowledge graph evolution during idle periods.
    
    \item \textbf{Multi-Agent Worktrees}: Git-inspired worktree isolation allowing concurrent agents to operate on independent memory branches without interference.
    
    \item \textbf{Complete Multi-Interface Support}: Native Rust library, PyO3 Python bindings, FastAPI REST server, React Web UI, and MCP (Model Context Protocol) adapter for Claude/Cursor integration.
\end{itemize}
