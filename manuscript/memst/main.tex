...
Our evaluation on the LOCOMO benchmark (1,986 questions across 10 long conversations) demonstrates that MemSt achieves \textbf{41.7\% F1}, an \textbf{84\% improvement} over RAG baselines (22.6\%) and outperforming published systems including Mem0 and Zep. MemSt achieves this with \textbf{3.2$\times$ lower latency} (85ms vs 280ms) and 95\% fewer tokens than full-context approaches. Specialized strategies excel in their domains: TemporalKG achieves 54.0\% F1 on temporal questions (+120\% vs RAG), while MultiHopKG achieves 52.8\% F1 on multi-hop questions (+133\% vs RAG).
